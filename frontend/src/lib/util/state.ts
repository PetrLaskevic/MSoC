...
export const loadState = (data: string): void => {
  ...
  try {
    //precte z url (decodes base64/pako) to mermaid text diagtam
    state = deserializeState(data);
    //if no state in URL, load some default
    if (!state.mermaid) {
      state.mermaid = defaultState.mermaid;
    }
    //converts the state to JSON object if its isnt
    ...
    //and makes it a string again
    //actual state update with the mermaid graph text we got
    //makes the state a string:
    //formatJSON = (data: unknown): string => JSON.stringify(data, undefined, 2) //2 inserts whitespace for readability
    state.mermaid = formatJSON(mermaidConfig);
  } catch (error) {
    state = get(inputStateStore);
    if (data) {
      //updates the state if error
